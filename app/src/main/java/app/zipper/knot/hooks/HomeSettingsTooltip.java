...
public class HomeSettingsTooltip implements BaseHook {
  ...
  public void hook(KnotConfig config, XC_LoadPackage.LoadPackageParam lpparam)
    ...
    if (cfg == null || cfg.res.resSettingsHeaderBtn.isEmpty() ||
        cfg.searchBarAgentI.homeSearchBarClass.isEmpty() ||
        cfg.searchBarAgentI.homeRefreshMethod.isEmpty())
      return;

    XposedHelpers.findAndHookMethod(
        cfg.searchBarAgentI.homeSearchBarClass, lpparam.classLoader,
        cfg.searchBarAgentI.homeRefreshMethod, new XC_MethodHook() {
          @Override
  ...
  private static void onHomeTabEntered(View homeView, LineVersion.Config cfg) {
    ...
    if (homeView.getGlobalVisibleRect(rect) && !rect.isEmpty()) {
      ...
      homeView.getViewTreeObserver().addOnGlobalLayoutListener(
  ...
  private static void triggerShow(View homeView, LineVersion.Config cfg) {
    if (!homeView.isShown() || !SettingsStore.isConfigured())
      return;
    ...
    host.runOnUiThread(() -> {
      if (activePopup != null && activePopup.isShowing())
        return;
      View btn = findViewByEntryName(host.getWindow().getDecorView(),
                                     cfg.res.resSettingsHeaderBtn);
      if (btn == null)
        return;
  ...
  private static void
  removeListener(View v, ViewTreeObserver.OnGlobalLayoutListener l) {
    try {
  ...
  private static View findViewByEntryName(View root, String entryName) {
    ...
    if (id != View.NO_ID) {
      try {
        if (entryName.equals(root.getResources().getResourceEntryName(id)))
          return root;
      } catch (Throwable ignored) {
      }
    }
  ...
  private static void showTooltip(Activity host, View anchor, View homeView) {
    try {
      ...
      int arrowId = host.getResources().getIdentifier(cfg.res.resTooltipArrowUp,
                                                      "drawable", pkg);
      if (arrowId != 0)
      ...
      arrowLp.rightMargin =
          Math.max(0, anchor.getWidth() / 2 - arrow.getMeasuredWidth() / 2);
      ...
      int bgId = host.getResources().getIdentifier(cfg.res.resTooltipBackground,
                                                   "drawable", pkg);
      if (bgId != 0)
        text.setBackgroundResource(bgId);
      ...
      PopupWindow popup =
          new PopupWindow(container, LinearLayout.LayoutParams.WRAP_CONTENT,
                          LinearLayout.LayoutParams.WRAP_CONTENT);
      ...
      popup.showAsDropDown(anchor,
                           anchor.getWidth() - container.getMeasuredWidth(), 0);
      activePopup = popup;

      homeView.getViewTreeObserver().addOnScrollChangedListener(
          new ViewTreeObserver.OnScrollChangedListener() {
            ...
            public void onScrollChanged() {
              ...
              if (!homeView.getGlobalVisibleRect(r) || r.isEmpty()) {
                dismissSilently();
                try {
                  homeView.getViewTreeObserver().removeOnScrollChangedListener(
                      this);
                } catch (Throwable ignored) {
                }
              }
